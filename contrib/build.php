#!/usr/bin/env php
<?php
namespace be\bastelstu\wcf\push;
/**
 * Builds push package.
 *
...
echo "Building Push $matches[1]\n";
echo str_repeat("=", strlen("Building Push $matches[1]"))."\n";
...
EOT;
	...
	foreach (glob('file/acp/be.bastelstu.wcf.push/lib/*.js') as $nodeFile) unlink($nodeFile);
	foreach (glob('file/js/*.js') as $jsFile) unlink($jsFile);
	if (file_exists('be.bastelstu.wcf.push.tar')) unlink('be.bastelstu.wcf.push.tar');
echo <<<EOT
...
Building be.bastelstu.wcf.push.tar
----------------------------------
...
EOT;
	...
	passthru('tar cvf be.bastelstu.wcf.push.tar --exclude=*.old --exclude-vcs --exclude=file --exclude=template --exclude=acptemplate --exclude=contrib -- *', $code);
	if (file_exists('package.xml.old')) {
		file_put_contents('package.xml', file_get_contents('package.xml.old'));
		unlink('package.xml.old');
	}
